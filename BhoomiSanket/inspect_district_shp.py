...
if os.path.exists(shp_path):
    ...
    state_col = None
    for col in ['STATE', 'ST_NM', 'State_Name', 'StateName', 'stname']:
    ...
    if state_col:
        ...
        if len(maharashtra_rows) > 0:
            print("Maharashtra found in district shapefile: YES")
            district_col = None
            for col in ['DISTRICT', 'DIST_NAME', 'District_Name', 'DistName', 'dtname', 'District']:
                if col in gdf.columns:
                    district_col = col
                    break
            if district_col:
                unique_districts = maharashtra_rows[district_col].unique()
                print("Maharashtra districts:", sorted(unique_districts))
            else:
                print("No district column found.")
        else:
            print("No Maharashtra found in district shapefile.")
    else:
    ...
    if state_col:
        ...
        print("All unique states:", sorted(unique_states))
else:
