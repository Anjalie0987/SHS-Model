...
class District(Base):
    ...
    summaries = relationship("DistrictSoilSummary", back_populates="district")
...
class SoilSample(Base):
    ...
    stage = Column(String(20)) # e.g. Germination, Booting, Ripening
...
class SoilHealthScore(Base):
    ...
    farm_id = Column(Integer, ForeignKey('farm.farm_id'))
    score_model_id = Column(Integer, ForeignKey('score_model.score_model_id'))
...
class DistrictSoilSummary(Base):
    ...
    district = relationship("District", back_populates="summaries")
