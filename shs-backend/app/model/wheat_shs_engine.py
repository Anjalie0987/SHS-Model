...
- Uses AHP for weighting
- Uses Fuzzy Membership Functions
- Supports manual input + dataset processing
- Returns SHS (0–100)
==========================================================
"""
...
class WheatSHSEngine:
    ...
    def validate_input(self, sample: Dict[str, Any]) -> None:
        missing = [field for field in self.criteria if field not in sample]
        if missing:
            raise ValueError(f"Missing field(s): {missing}")

        for field in self.criteria:
            value = sample[field]
            if pd.isna(value):
    ...
    def run_dataset(self, df: pd.DataFrame) -> pd.DataFrame:
        result_df = df.copy()

        required = self.required_fields()
        missing_cols = [col for col in required if col not in result_df.columns]
        if missing_cols:
        ...
        for _, row in result_df.iterrows():
            ...
            try:
                result = self.predict(row_dict)
                scores.append(result["SHS"])
                errors.append(None)
            except Exception as exc:
                scores.append(None)
                errors.append(str(exc))

        result_df[f"SHS_{self.stage}"] = scores
        result_df[f"Error_{self.stage}"] = errors
